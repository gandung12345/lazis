...
use Lazis\Api\Repository\Observer\DsklObserverRepository;
 ...
 */
class DsklObserverController extends BaseController
{
    use ControllerTrait;

    #[Route(
        '/observer/dskl/{scope}/{district}/{village}',
        method: 'GET'
    )]
    #[OpenApi\Get(
        path: '/observer/dskl/{scope}/{district}/{village}',
        tags: ['Observer'],
        responses: [
            new OpenApi\Response(response: 200, description: 'OK')
        ]
    )]
    public function getAllDskls(
        Request $request,
        Response $response,
        array $args
    ): Response {
        $repository = new DsklObserverRepository(
