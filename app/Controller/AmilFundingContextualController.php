...
use Lazis\Api\Repository\AmilFundingContextualRepository;
 ...
 */
class AmilFundingContextualController extends BaseController
{
    use ControllerTrait;
    ...
    #[Route('/organizationContext/{oid}/amilFunding', method: 'GET')]
    #[OpenApi\Get(
        path: '/organizationContext/{oid}/amilFunding',
        tags: ['Amil Funding Context'],
        responses: [
            new OpenApi\Response(response: 200, description: 'OK'),
            new OpenApi\Response(response: 400, description: 'Bad Request')
        ]
    )]
    public function getAllAmilFundings(
        Request $request,
        Response $response,
        array $args
    ): Response {
        $repository = new AmilFundingContextualRepository(
