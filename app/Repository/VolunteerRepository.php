...
use Lazis\Api\Entity\VolunteerBulkResponseState;
...
class VolunteerRepository extends AbstractRepository
{
     ...
     */
    public function getById($id, bool $hydrated = true): EntityInterface|null|array
    {
        $entity = $this->getMapper()->find(new Volunteer(), $id);
    ...
    public function create(
        ...
        $volunteer->setId(Uuid::v7()->toString());
    ...
    public function createBulk(
        ...
        foreach ($schemas as $key => $schema) {
            try {
                $volunteer = $this->create($refId, $schema, false);
                $result[] = $this->createVolunteerStateObject(
                    $key,
                    $volunteer->getId(),
                    HttpCode::CREATED,
                    sprintf(
                        'Volunteer created with id \'%s\'.',
                        $volunteer->getId()
                    )
                );
            } catch (Throwable $e) {
    ...
    public function update($id, SchemaInterface $schema): EntityInterface|null|array
    {
        $result = $this->getMapper()
            ...
            ->update($id, $schema, new Volunteer());
     ...
     */
    private function createVolunteerStateObject(
        int $index,
        string $id,
        int $status,
        string $message
    ): EntityInterface {
        $state = new VolunteerBulkResponseState();
