 ...
 */
class OrganizationObserverRepository extends AbstractRepository
{
    use RepositoryTrait;
    ...
    public function count(int $scope, string $district, string $village): int
    {
        ...
        $queryBuilder = $queryBuilder
            ->select(
                sprintf(
                    'count(%s)',
                    $organization->getQueryBuilderAlias()
                )
            )
            ->from($organization->getDqlName(), $organization->getQueryBuilderAlias())
            ->where($this->buildObserverPredicate($queryBuilder, $organization))
    ...
    private function resolvePaginationByMultipleConstraint(
        ...
        return $this->getMapper()
            ...
            ->withPage($page)
            ->paginateFromQueryBuilder($queryBuilder, $organization);
    }
}
